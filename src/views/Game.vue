<template>
    <div class="w-full h-screen flex items-center justify-center">
        <div class="relative w-full h-1/3 rounded-2xl text-4xl text-base-100 font-bold flex items-center flex-col bg-accent"
            ...
            <p v-if="this.current.role != 'undercover' && this.current.role != 'white' && this.current.role != 'next'">Your
                Word: {{ word }}</p>
            <p v-if="this.current.role == 'undercover'">Your Word: {{ word2 }}</p>
            <p v-if="this.current.role == 'white'" class="text-6xl">Mr. WHITE</p>
            <p v-if="this.current.role == 'next'" class="text-6xl">NEXT</p>
...
export default {
    methods: {
        next() {
            ...
            let tmp = null
            if (this.x % 2 == 0) {
                tmp = { name: "Give next player: " + this.remaining[this.remaining.length - 1].name, role: "next" }
            }
            else {
                tmp = this.remaining.pop()
            }
